...
for filenumber=1:length(vesszohely)+1
    ...
    todel=[];
    for seriesi=1:length(potseriesnums) %modifying seriesdata&seriesnums to include only the needed channel
        neededchannel=find(seriesdata(potseriesnums(seriesi)).tracenum==channel);
        if isempty(neededchannel)
            todel=[todel,seriesi];
        else
            seriesdata(potseriesnums(seriesi)).tracenum=seriesdata(potseriesnums(seriesi)).tracenum(neededchannel);
            seriesdata(potseriesnums(seriesi)).tracename=seriesdata(potseriesnums(seriesi)).tracename(neededchannel);
            seriesnums(potseriesnums(seriesi),4)=1;
        end
    end
    potseriesnums(todel)=[];
    neededseriesnums=potseriesnums;
    rawdatanew=HEKAexportbytreeinfo_main(fname,setupname,seriesnums,seriesdata,neededseriesnums);
    rawdata=[rawdata,rawdatanew];
    
% % % % % % %     %% ARCHIVED - merged with HEKAexportbytreeinfo_main
