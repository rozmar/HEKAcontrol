...
tic;
timepassed=0;
while isempty(tempp) | str2num(char(tempp(1)))<0 | abs(str2num(char(tempp(1))))==abs(lastsignature) | (length(tempp)<2 & timepassed<3 & ~any(strfind(order,'@')) & ~any(strfind(order,' O '))& ~any(strfind(order,'OpenOnline')))%% megvárja míg az output file signature-ja megváltozik és pozitív előjelű nem lesz - 
